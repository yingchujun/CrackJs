...
def get_data(page, cookies):
    ...
    response = requests.get(url, headers=headers, cookies=cookies, params=params, proxies=proxy, verify=False)
...
def get_login_cookie(username, password):
    ...
    response = requests.post(login_url, data=data, proxies=proxy, verify=False)
