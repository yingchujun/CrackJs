...
def get_page_count(page,cookie=''):
    url = 'https://match.yuanrenxue.cn/api/match/4'
    data = {
        'page' : page
    }
    response = requests.get(url, params=data, cookies=cookie).json()
    key = response['key']
    ...
    tds = html.xpath('//td')
    # 获取隐藏的图片id
    classId = md5(strToBase64(key + value).replace('=', ''))
    count =0 
    for td in tds:
        num_dict_list = []
        numSrcList = td.xpath("./img[not(contains(@class, '{}'))]/@src".format(classId))
        numStyleList = td.xpath("./img[not(contains(@class, '{}'))]/@style".format(classId))
        for index, item in enumerate(numStyleList):
            temp = {}
            position = float(item.replace('left:', '').replace('px', ''))
            temp[srcToNumList[numSrcList[index]]] = position
        ...
        for index, val in enumerate(num_dict_list):
            ...
            num_list[key] = str(list(val.keys())[0])
...
if __name__ == '__main__':
    ...
    count = 0
    for i in range(1, 6):
        result = get_page_count(i, cookies)
        count += result
    print('count:', count)
